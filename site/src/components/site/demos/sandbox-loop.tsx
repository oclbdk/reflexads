...
// The Contributor's Guide's resident toy: the 1.4 event-loop demo with a
// smiley preloaded, parked in step 03 with no explanation — it's there to
// play with, and to be customized or replaced in a fork. Boots at WFI (not
// CLR) so the smiley survives until someone decides otherwise.
...
const SMILEY: ReadonlySet<string> = new Set(
  [
    [2, 2], [5, 2],
    [1, 4], [6, 4], [2, 5], [3, 5], [4, 5], [5, 5],
  ].map(([x, y]) => `${x},${y}`),
)
...
const INITIAL: Sim = { pc: 1, cycles: 0, lit: SMILEY, regs: null, delivering: false }
...
const nodeTypes = {
  sandboxStream: StreamNode,
  sandboxCpu: CpuNode,
  sandboxController: ControllerNode,
  sandboxDisplay: TouchDisplayNode,
}

// ---- the demo ---------------------------------------------------------

export function SandboxLoopDemo() {
