...
yours to choose, the same freedom you use every time you pull three steps into
a helper or fold two into one and never wonder whether the meaning survived.
What that freedom does _not_ cover is order: regrouping a run of interactions
is safe, reordering it is not, and in any domain where sequence carries meaning
that is the exact line a refactoring may not cross. So a monoid is two things
at once: a set of interactions, and the conditions those interactions satisfy.
That is precisely why it models the Ground: it is an interaction space that
_carries its own conditions_, bundled into a single object, which is the
property this whole book is chasing.
...
Reflad on a Ground. For now the reflexad has just the one face, its reflad, the
owning half. It is built to _stand on_ a ground rather than to _contain_ one
for a reason: the ground is meant to be shared. A second face will stand on the
very same ground in a later chapter, and the reflexad will become the pair.
...
Read that line for what it says. The reflad's associativity, the monad law, is
nothing but the monoid's associativity, handed up from the Ground it stands on.
The structure does not _assume_ its central condition; it _gets_ it, from the
ground it owns. (The unit laws come the same way.) This is the first concrete
thing we can point at when we say a structure hosts its own conditions: the
condition is sourced from the ground, not decreed from outside. And that
already tells you where a domain's invariants belong: not scattered across the
operations that touch the context, re-checked at each call site, but
concentrated once in the algebra of the ground, where everything that
...
instance, one law and one ground, but it is real, and it is the shape the rest
of the book keeps widening.
