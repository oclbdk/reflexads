...
behind* — the value it computed, the connection it opened, the error state
it might already be in. Each step is written against a context the steps
before it built up. We write code like this so constantly it feels like
nothing.
...
wrapping, its own flattening — and it turned up exactly when you leaned on
it.
...
can always take as its own — is what we'll call a *reflexad*. The name is just that:
*reflex-* for the turn back on itself, *-ad* from the monad it never stops
being.

Nothing about the machinery changes. A reflexad is not a weaker or a
stronger monad; it's an ordinary monad, looked at from the angle
this book takes. Written out as code, that's the whole of the claim —
a reflexad *is* a monad, with the two operations this book cares about named
out front:

#agda("../../src/Reflexads/Chapter01.agda", tag: "reflexad")

The `monad` it's built on — and the laws that monad keeps about itself —
sits right beside this in the same module; `own` and `flatten` add nothing
to it but a change of angle — two names for what that angle brings forward:
...
throughout.
Whenever we want to probe how a structure can host its own conditions — how
it can sequence itself in its own context, start to finish — we'll reach for
the reflexad, because it's the smallest and most
familiar place that self-contextualization already lives.
...
instinct. The
cyclic dependency chains you fight aren't the disease; they're what
contextualized sequencing looks like when the structure inducing it can't
hold itself up. So the fix is not a blanket "break the cycle." It's to see
the sequencing underneath, and then to build — or recognize — structures
