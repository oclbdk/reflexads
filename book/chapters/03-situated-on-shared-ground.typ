...
always read the value that resides here, at this position, and lay out the whole
surrounding context from where you stand. What you cannot do, in general, is
inject. You do not make context; you _host_ it. That stance has a structure
too, the exact mirror of the monad, and it is called a *comonad*.

#agda("../../src/Reflexads/Chapter03.agda", tag: "comonad")

Its two operations are the monad's, dualized. Where the monad had `return` (put
a value in) the comonad has `extract`: read the value residing here. Where the
monad had bind (sequence a step onto the context it owns) the comonad has
...
A monad and a comonad sharing nothing. So ask what the layer adds, because that
forgetful map throws away exactly what the layer adds. And it adds two things.
